<div class="relative">
    <div class="relative z-10">
        <a href="{{route('course-detail' , $course)}}" class="block">
            <img src="{{Storage::url($course->image)}}" class="max-w-full rounded-3xl"
                 alt="{{$course->title}}" />
        </a>
    ...
    <div class="bg-background rounded-b-3xl -mt-12 pt-12">
        <div
           ...
           $user = auth()->user();
           $courseHas = $user->activeCoursesFromLevel()->where('slug', $courseSlug)->first();
                @endphp
                @if($courseHas)
                    <span class="block w-1 h-1 bg-success rounded-full"></span>
                    <span class="font-bold text-xs text-success">دسترسی دارید</span>
                @else
                    <span class="block w-1 h-1 bg-red-500 rounded-full"></span>
                    <span class="font-bold text-xs text-red-500">دسترسی ندارید!</span>
                @endif
        ...
        <div class="space-y-3 p-5">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex items-center gap-1 text-muted">
                    ...
                    <span class="font-semibold text-xs">{{ gmdate('H:i:s', ($course->total_time ?? 0) * 60) }}</span>
                </div>
            </div>
